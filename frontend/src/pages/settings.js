...
export default function Settings() {
  ...
  async function handleShare(event) {
    await apiNoResponse("/friends/access/toggle", {
      method: "POST",
  ...
  useEffect(() => {
    if (user) setAllowFriendsAccess(user.allowsFriendlistAccess);
  }, [setAllowFriendsAccess, user]);

  let tooltipTitle = "";
  if (user)
    tooltipTitle = t("refresh-tooltip", {
      updatedAgo: formatDistance(new Date(user.updatedAt), new Date(), {
        addSuffix: true,
      }),
    });
        ...
        <>
          ...
          <Box sx={{ display: "flex", flexWrap: "wrap", mt: 2 }}>
            <FormControlLabel
              control={
                <Switch
                  defaultChecked={user.allowsFriendlistAccess}
...

            {
              <TextField
                ...
                sx={{
                  flexGrow: 1,
                  display: allowFriendsAccess ? "inherit" : "none",
                }}
